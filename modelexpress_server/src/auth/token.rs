...
//! Bearer-token extraction and Kubernetes `TokenReview` verification. Extracts the bound
//! pod name/uid the device check relies on.
...
/// `extra` keys the apiserver populates for bound projected tokens (k8s >= 1.22).
...
impl TokenError {
    /// True when the token itself is bad (safe to cache); false for transient API errors.
...
/// Parse `system:serviceaccount:<ns>:<sa>`.
...
/// Returns `None` for non-service-account principals (nodes, users).
...
/// `audiences` must match what the caller's projected token was minted for; passing the
/// wrong audience can make the apiserver accept tokens it should reject. Enforce mode
/// requires a non-empty audience list at config-load time.
pub(crate) async fn review_token(
    ...
    // TokenReview is a write-only subresource: `create` is its POST verb.
    let reviewed = api
