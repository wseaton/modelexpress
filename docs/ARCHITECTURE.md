...
| `AdvertiseTensorCatalog` | `AdvertiseTensorCatalogRequest` | `AdvertiseTensorCatalogResponse` | Persist a generation-tracked lightweight tensor catalog for planning |
| `ComputeTransferPlan` | `ComputeTransferPlanRequest` | `ComputeTransferPlanResponse` | Assign requested tensors across eligible source peers |

Key message types: `SourceIdentity` (all fields affecting tensor layout compatibility), `WorkerMetadata` (rank, oneof backend_metadata, tensors, status, P2P endpoint fields), `TensorDescriptor` (name, addr, size, device_id, dtype), `TensorCatalogEntry` (name, byte length, dtype, optional shape, no GPU address), `SourceInstanceRef` (lightweight worker reference for listing).
...
- **Redis** (`redis`): Source index hashes (`mx:source:{source_id}`) with an `__attributes__` field storing `SourceIdentity` and `{worker_id}` fields as presence markers. Worker data stored in separate hashes (`mx:source:{source_id}:{worker_id}`); lightweight tensor catalogs are stored inline on each worker record with a monotonic generation. Stale detection and cleanup handled by the server-side reaper.
- **Kubernetes** (`kubernetes`/`k8s`/`crd`): `ModelMetadata` CRDs (one per worker) with `ConfigMap`s for tensor descriptors and lightweight tensor catalogs. Catalog ConfigMap names and generations are stored in `status.worker`, and owner references provide automatic garbage collection. Standard Kubernetes `status.conditions` (`Ready`) and `status.observedGeneration` are maintained so that `kubectl wait --for=condition=Ready` works. Stale detection handled by the server-side reaper.
...
2. **Source publishes**: Registers tensors with NIXL, calls `PublishMetadata(identity, worker, worker_id)` -> gets `mx_source_id` (status=INITIALIZING). In P2P mode (`MX_P2P_METADATA=1`, or auto-forced on by decentralized backends like `k8s-service`), publishes only lightweight endpoint pointers and starts a `WorkerGrpcServer` for tensor manifest serving. When transfer planning is enabled, the source also calls `AdvertiseTensorCatalog` so the server can persist the owned tensor names, sizes, dtypes, and catalog generation without GPU addresses.
...
5. **Target plans and fetches on demand**: Calls `ComputeTransferPlan` when multi-peer planning is enabled, otherwise calls `GetMetadata(mx_source_id, worker_id)` for the chosen candidate. The planner resolves the need-set per the proto contract: explicit `requested_tensor_names` win; an empty list uses the requester's own advertised catalog (the partial/resume/diff path, where the receiver advertised via `PublishMetadata(INITIALIZING)` + `AdvertiseTensorCatalog` before planning); and the union of all peer catalogs is the last resort when the requester advertised nothing (the cold-start full-load case, noted in `PlanDiagnostics`). It then greedily assigns each needed tensor to the least-loaded owning peer, dropping any tensor whose peers (or the requester) disagree on dtype or shape into `uncovered` rather than transferring mismatched bytes. The planner uses persisted tensor catalogs when present and falls back to full tensor descriptors for older peers. Auto-detects P2P mode if `worker_grpc_endpoint` is populated - fetches tensors from the source worker's `WorkerService` and NIXL metadata via the listen thread instead of from the central server.
