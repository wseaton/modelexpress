...
| `AdvertiseInventory` | `AdvertiseInventoryRequest` | `AdvertiseInventoryResponse` | Persist a generation-tracked owned-tensor inventory for planning |
...
- **Redis** (`redis`): Source index hashes (`mx:source:{source_id}`) with an `__attributes__` field storing `SourceIdentity` and `{worker_id}` fields as presence markers. Worker data stored in separate hashes (`mx:source:{source_id}:{worker_id}`); inventories are stored inline on each worker record with a monotonic generation. Stale detection and cleanup handled by the server-side reaper.
- **Kubernetes** (`kubernetes`/`k8s`/`crd`): `ModelMetadata` CRDs (one per worker) with `ConfigMap`s for tensor descriptors and inventories. Inventory ConfigMap names and generations are stored in `status.worker`, and owner references provide automatic garbage collection. Standard Kubernetes `status.conditions` (`Ready`) and `status.observedGeneration` are maintained so that `kubectl wait --for=condition=Ready` works. Stale detection handled by the server-side reaper.
...
2. **Source publishes**: Registers tensors with NIXL, calls `PublishMetadata(identity, worker, worker_id)` -> gets `mx_source_id` (status=INITIALIZING). In P2P mode (`MX_P2P_METADATA=1`, or auto-forced on by decentralized backends like `k8s-service`), publishes only lightweight endpoint pointers and starts a `WorkerGrpcServer` for tensor manifest serving. When transfer planning is enabled, the source also calls `AdvertiseInventory` so the server can persist the owned tensor names, sizes, dtypes, and inventory generation without GPU addresses. A peer is only assignable in a plan if it has advertised an inventory.
...
5. **Target plans and fetches on demand**: Calls `ComputeTransferPlan` when multi-peer planning is enabled, otherwise calls `GetMetadata(mx_source_id, worker_id)` for the chosen candidate. The planner resolves the need-set from `requested_tensor_names`: an explicit list restricts assignments to that set; an empty list uses the union of all peers' advertised inventories (the cold-start full-load case). Peer ownership comes solely from advertised inventories, so a peer that never called `AdvertiseInventory` is not assignable. It then greedily assigns each needed tensor to the least-loaded owning peer, dropping any tensor whose peers disagree on dtype into `uncovered` rather than transferring mismatched bytes. Auto-detects P2P mode if `worker_grpc_endpoint` is populated - fetches tensors from the source worker's `WorkerService` and NIXL metadata via the listen thread instead of from the central server.
