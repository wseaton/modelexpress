...
"""Tests for the bearer-token client auth interceptor.

No mocks: token reads hit a real temp file and the interceptor is exercised against a
real in-process gRPC server that echoes back the metadata it received.
"""
...
from modelexpress.auth import TokenProvider, create_channel, with_auth
...
def test_create_channel_attaches_auth():
    # create_channel builds a plaintext channel and attaches the bearer-token interceptor;
    # the token is absent here so the call reaches the server without an authorization header.
    handler = _CaptureHandler()
    server, port = _serve(handler)
    try:
