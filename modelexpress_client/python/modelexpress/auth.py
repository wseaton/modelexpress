...
"""Bearer-token auth for ModelExpress gRPC clients.

Reads a Kubernetes projected ServiceAccount token from a file and attaches it as
``authorization: Bearer <token>`` on every outbound RPC. Re-read on TTL/mtime change.
The token value is never logged. With no token file present the interceptor is a no-op
(server must be ``off``/``permissive``).
"""
...
class TokenProvider:
    """Caches a projected ServiceAccount token; re-reads on TTL expiry or mtime change.

    Thread-safe: the interceptor may be invoked concurrently from worker threads sharing
    one channel.
    """
    ...
    def get(self) -> str | None:
        ...
        with self._lock:
            ...
            try:
                ...
                # Past TTL: re-stat and only re-read when mtime actually changed.
                if (
                ...
                if not self._warned_missing:
                    logger.warning(
                        ...
                        "(server must be in off/permissive mode).",
                        self._path,
                    )
...
def with_auth(channel: grpc.Channel, provider: TokenProvider | None = None) -> grpc.Channel:
    """Wrap ``channel`` so all RPCs carry the bearer token (no-op when absent)."""
...
def create_channel(
    ...
    """Single entry point: plaintext channel + bearer-token interceptor."""
    return with_auth(grpc.insecure_channel(target, options=options or []))
