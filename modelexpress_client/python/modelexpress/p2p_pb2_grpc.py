...
class P2pServiceStub(object):
    ...
    def __init__(self, channel):
        ...
        self.ComputeTransferPlan = channel.unary_unary(
                ...
                _registered_method=True)
        self.AdvertiseInventory = channel.unary_unary(
                '/model_express.p2p.P2pService/AdvertiseInventory',
                request_serializer=p2p__pb2.AdvertiseInventoryRequest.SerializeToString,
                response_deserializer=p2p__pb2.AdvertiseInventoryResponse.FromString,
                _registered_method=True)
...
class P2pServiceServicer(object):
    ...
    def AdvertiseInventory(self, request, context):
        """Advertise this worker's owned-tensor inventory for planning.

        Lightweight entries (name + dtype + byte_len + optional shape; no GPU
        addresses) used by the planner to match peers that own a needed tensor.
        Required: the server cannot derive ownership from SourceIdentity alone
        (peers may own disjoint shards — MoE with expert-parallel, asymmetric
        quantization variants, etc.), so a peer that has not advertised an
        inventory is not eligible to serve any tensor in a plan.

        Sent once per peer after the model is loaded and before the peer is
        considered eligible for transfer planning. Sources advertise after
        load; receivers re-advertise after their own transfer completes so
        they can serve future requesters.

        Re-sending replaces the previous inventory for (mx_source_id, worker_id)
...
def add_P2pServiceServicer_to_server(servicer, server):
    rpc_method_handlers = {
            ...
            'AdvertiseInventory': grpc.unary_unary_rpc_method_handler(
                    servicer.AdvertiseInventory,
                    request_deserializer=p2p__pb2.AdvertiseInventoryRequest.FromString,
                    response_serializer=p2p__pb2.AdvertiseInventoryResponse.SerializeToString,
            ),
    }
...
class P2pService(object):
    ...
    @staticmethod
    def AdvertiseInventory(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
        ...
        return grpc.experimental.unary_unary(
            ...
            target,
            '/model_express.p2p.P2pService/AdvertiseInventory',
            p2p__pb2.AdvertiseInventoryRequest.SerializeToString,
            p2p__pb2.AdvertiseInventoryResponse.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
